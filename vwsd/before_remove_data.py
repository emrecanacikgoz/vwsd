...
class VWSDDataset(Dataset):
	...
	# TODO randomly add items and labels as seen here
        ...
        self.items = []
        scraped_items, scraped_val_items = self._read_scraped_data()
		...
		while item["image_path"] in negatives:
                ...
                word, context = item["nl_context"].split()
                negatives.insert(0, context)
                negatives.insert(0, word)
                items.append(negatives) # TODO
                labels.append(item["image_path"])

        val_items, val_labels, all_val_img_paths = [], [], []
        
        if scraped_val_items is not None:
            for item in scraped_val_items:
                if "language" in item: # if the example is from WiT
                    all_val_img_paths.append(item["image_path"])

            for item in scraped_val_items:
                if "language" in item:
                    negatives = random.sample(all_val_img_paths, 9)
		    while item["image_path"] in negatives:
                         negatives = random.sample(all_val_img_paths, 9)
                    ...
                    negatives.insert(0, context)
                    negatives.insert(0, word)
                    val_items.append(negatives) # TODO
                    val_labels.append(item["image_path"])
        ...
        if self.debug:
            ...
            if scraped_val_items is not None:
                val_items, val_labels, scraped_val_items = val_items[:40], val_labels[:40], scraped_val_items[:40]
        ...
        if scraped_val_items is not None:
            for inputs, label, scraped_item in zip(val_items, val_labels, scraped_val_items):
                complete_context = f"{inputs[0]} {inputs[1]}"
                prompt = self._clean_scraped_data(scraped_item, complete_context)
    ...
    def _read_scraped_data(self):
        print(os.getcwd())
        val_path, val_items = None, None
        if self.split == "train":
            file_path = "../vwsd/data.json"
            val_path = "../vwsd/data_val.json"
        elif self.split == "trial":
            file_path = "../vwsd/data_trial.json"
        elif self.split == "test":
            file_path = "../vwsd/data_test.json"

        with open(file_path, 'r') as scraped_data:
            scraped_items = list(scraped_data)

        if val_path is not None:
            with open(val_path, "r") as val_data:
                scraped_val_items = list(val_data)
            val_items = [json.loads(item) for item in scraped_val_items]
        
        return [json.loads(item) for item in scraped_items], val_items
